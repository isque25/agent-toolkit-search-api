...
class AgentToolkit(SyncAPIClient):
    ...
    def __init__(
        ...
        *,
        ...
        """Construct a new synchronous AgentToolkit client instance.

        This automatically infers the `api_key` argument from the `AGENT_TOOLKIT_API_KEY` environment variable if it is not provided.
        """
        if api_key is None:
            api_key = os.environ.get("AGENT_TOOLKIT_API_KEY")
        if api_key is None:
            raise AgentToolkitError(
                "The api_key client option must be set either by passing api_key to the client or by setting the AGENT_TOOLKIT_API_KEY environment variable"
            )
        self.api_key = api_key

        if base_url is None:
            base_url = os.environ.get("AGENT_TOOLKIT_BASE_URL")
        if base_url is None:
            base_url = f"https://api.agenttoolkit.ai"

        custom_headers_env = os.environ.get("AGENT_TOOLKIT_CUSTOM_HEADERS")
...
class AsyncAgentToolkit(AsyncAPIClient):
    ...
    def __init__(
        ...
        *,
        ...
        """Construct a new async AsyncAgentToolkit client instance.
