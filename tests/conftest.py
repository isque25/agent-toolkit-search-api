...
from agent_toolkit import AgentToolkit, AsyncAgentToolkit, DefaultAioHttpClient
from agent_toolkit._utils import is_dict
...
def client(request: FixtureRequest) -> Iterator[AgentToolkit]:
    strict = getattr(request, "param", True)
    if not isinstance(strict, bool):
        raise TypeError(f"Unexpected fixture parameter type {type(strict)}, expected {bool}")

    with AgentToolkit(base_url=base_url, api_key=api_key, _strict_response_validation=strict) as client:
        yield client


@pytest.fixture(scope="session")
async def async_client(request: FixtureRequest) -> AsyncIterator[AsyncAgentToolkit]:
